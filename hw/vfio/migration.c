                                    ...
                                    enum vfio_device_mig_state recover_state)
{
    ...
    struct vfio_device_feature_mig_state *set_state = (void *)feature->data;
    int ret;

    feature->argsz = sizeof(buf);
    feature->flags = VFIO_DEVICE_FEATURE_SET | VFIO_DEVICE_FEATURE_MIG_DEVICE_STATE;
    set_state->device_state = new_state;
    ...
    if (ret) {
        ...
        if (ioctl(vbasedev->fd, VFIO_DEVICE_FEATURE, feature)) {
            ...
            hw_error("%s: Device in error state, can't recover", vbasedev->name);
            return -1;
        }
    ...
    if (set_state->data_fd != -1) {
        if (migration->data_fd != -1) {
             ...
             * terminates a PRE_COPY data transfer.
             */
